...
try {
    ...
    const body = {
        ...
        ...(pageDiagnostics.diagnosticsRetention
            ? { diagnostics_retention: pageDiagnostics.diagnosticsRetention }
            : {}),
        requests,
...
function buildDiagnosedRequest({
    clientRequestId,
    feedback,
    diagnostics,
    diagnosticsCapabilities,
...
function buildDiagnosticsNoMatchNote(diagnostics, diagnosticsCapabilities, diagnosticsRetention) {
    ...
    const retention =
        readObject(diagnostics.diagnostics_retention) ??
        readObject(diagnosticsCapabilities?.retention) ??
        diagnosticsRetention;
    return {
...
function buildDiagnosticsNoMatchMessage(retention) {
    const windowText =
        retention && Number.isInteger(retention.max_entries)
            ? `最近 ${retention.max_entries} 条本地应用日志`
            : '当前本地应用日志保留窗口';
...
function printUsage() {
    console.error(
        '用法：diagnose-request.mjs [--base-url <url>] [--client-request-id <id>] [--agent-request-id <id>] [--idempotency-key <key>] [--manifest <jsonl>] [--filename <name>] [--output <json>]'
    );
