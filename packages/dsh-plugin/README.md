...
默认服务地址是 `http://localhost:4783`，也可以使用 `GPT_IMAGE_PLAYGROUND_URL` 或 profile patch 的 `baseUrl` 覆盖。鉴权使用 `GPT_IMAGE_AGENT_TOKEN`，或使用 `GPT_IMAGE_APP_PASSWORD_HASH` 发送 `X-App-Password-Hash`。密钥只从进程环境读取，不写入请求正文、日志或仓库文件。
